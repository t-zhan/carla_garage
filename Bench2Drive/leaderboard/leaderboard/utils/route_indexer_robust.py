...
class RouteIndexer():
    ...
    def validate_and_resume(self, endpoint):
        ...
        while check_index < resume_index:
            try:
                ...
                if route_data[check_index]['status'] not in ['Failed', 'Failed - Simulation crashed', 'Failed - Agent crashed', 'Failed - Simulation crashed']:
                    check_index += 1
                elif route_data[check_index]['status'] == 'Failed - Simulation crashed' and check_index < resume_index - 1:
                    check_index += 1
                else:
        ...
        import os
        max_route_tries = int(os.environ.get('MAX_ROUTE_TRIES'))
        route_tries = int(os.environ.get('ROUTE_TRIES')) + 1
        if route_tries > max_route_tries:
            print("======================================================")
            print(f"ROUTE_TRIES {route_tries} exceeds the maximum of {max_route_tries} tries.")
            print("Reset ROUTE_TRIES to 1 and moving to the next route.")
            print("======================================================")
            self.index += 1
            os.environ['ROUTE_TRIES'] = '1'
        return True
