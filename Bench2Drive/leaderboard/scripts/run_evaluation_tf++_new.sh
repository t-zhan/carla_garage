...
TEAM_AGENT=/home/autodrive/Projects/carla_garage/team_code/sensor_agent.py
# Must set YOUR_CKPT_PATH
TEAM_CONFIG=/home/autodrive/Projects/carla_garage/pretrained_models/all_towns
...
for ((i=0; i<$length; i++ )); do
      ...
      echo -e "\033[32m bash ${WORK_DIR}/leaderboard/scripts/run_evaluation_with_recovery_tf++.sh $PORT $TM_PORT $IS_BENCH2DRIVE $ROUTES $TEAM_AGENT $TEAM_CONFIG $CHECKPOINT_ENDPOINT $SAVE_PATH $PLANNER_TYPE $GPU_RANK \033[0m"
      echo -e "***********************************************************************************"
      bash ${WORK_DIR}/leaderboard/scripts/run_evaluation_with_recovery_tf++.sh $PORT $TM_PORT $IS_BENCH2DRIVE $ROUTES $TEAM_AGENT $TEAM_CONFIG $CHECKPOINT_ENDPOINT $SAVE_PATH $PLANNER_TYPE $GPU_RANK 2>&1 > ${BASE_ROUTES}_${TASK_LIST[$i]}_${ALGO}_${PLANNER_TYPE}.log &
      sleep 5
done
wait