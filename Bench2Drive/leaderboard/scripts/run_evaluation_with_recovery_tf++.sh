...
cleanup() {
    ...
    # 终止该 GPU 上的 CARLA 进程
    ps -ef | grep "graphicsadapter=${GPU_RANK}" | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null
    # 终止相关端口的进程
