...
class SensorAgent(autonomous_agent.AutonomousAgent):
  ...
  def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
    ...
    with open(os.path.join(self.config_path, 'config_max.json'), 'rt', encoding='utf-8') as f:
      json_config = f.read()
    ...
    #     state_dict = torch.load(os.path.join(self.config_path, file), map_location=self.device, weights_only=False)
    ...
    ## 1. load LidarCenterNet to infer bounding boxes and semantic segmentation.
    self.lcn_config = deepcopy(self.config) 
    self.lcn_config.__dict__.update({
      'use_wp_gru': 0,
    ...
    for file in os.listdir(self.config_path):
      if file.endswith('.pth') and file.startswith('model'):
        net = LidarCenterNet(self.lcn_config)
        if self.config.sync_batch_norm:
          # Model was trained with Sync. Batch Norm.
          # Need to convert it otherwise parameters will load wrong.
          net = torch.nn.SyncBatchNorm.convert_sync_batchnorm(net)
    ...
    ## 2. load Max to infer waypoints.
    net = Max.from_pretrained(self.config.model_name_or_path)
    net.config.carla_config = self.config
    if self.config.sync_batch_norm:
      net = torch.nn.SyncBatchNorm.convert_sync_batchnorm(net)
    net.eval()
    if self.config.compile or self.compile:
    ...
    self.model_count = len(self.nets)
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    ## 1. load LidarCenterNet to infer bounding boxes and semantic segmentation.
    _, _, _, pred_semantic, pred_bev_semantic, pred_depth, pred_bb_features, _, _, _ = self.nets[0].forward(
      rgb=tick_data['rgb'],
      lidar_bev=lidar_bev,
    ...
    pred_wp_1,\
    selected_path, \
    command_text = self.nets[1].carla_generate(
      rgb=tick_data['rgb'],
      # lidar_bev=lidar_bev,
      # target_point=tick_data['target_point'],
      # target_point_next=tick_data['target_point_next'] if self.config.two_tp_input else None,
    ...
    if self.lcn_config.detect_boxes and (compute_debug_output or self.config.backbone in ('aim') or
                                      self.stop_sign_controller):
      pred_bounding_box = self.nets[0].convert_features_to_bb_metric(pred_bb_features)
    else:
    ...
    if compute_debug_output:
      ...
      self.nets[1].visualize_model(
          self.save_path,
          self.step,
          tick_data['rgb'],
          lidar_bev,
          None, # tick_data['target_point'],
          pred_wp,
          None, # target_point_next=tick_data['target_point_next'] if self.config.two_tp_input else None,
    ...
    if self.config.inference_direct_controller and self.config.use_controller_input_prediction:
      ...
      steer, throttle, brake = self.nets[1].control_pid(self.pred_wp,
                                                        gt_velocity,
